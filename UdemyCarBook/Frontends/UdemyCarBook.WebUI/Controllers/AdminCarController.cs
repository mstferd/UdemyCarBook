...
namespace UdemyCarBook.WebUI.Controllers
{
    public class AdminCarController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage2 = await client.GetAsync("https://localhost:7125/api/Brands");
        ...
        public async Task<IActionResult> CreateCar(CreateCarDto createCarDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7125/api/Cars", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateCar(int id)
        {
            ...
            ViewBag.BrandValues = brandValues;
            var responseMessage = await client.GetAsync($"https://localhost:7125/api/Cars/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateCar(UpdateCarDto updateCarDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync($"https://localhost:7125/api/Cars", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
